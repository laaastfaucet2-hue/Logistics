...
@pytest.mark.parametrize('path', ['/dashboard','/rations/tamween','/rations/tamween?tab=dist',
    '/rations/contractor?tab=dist','/letterhead/',
    '/recruits/','/recruits/?tab=journal','/recruits/?tab=present','/recruits/?tab=leaves',
    '/recruits/?tab=absent','/recruits/?tab=other','/recruits/?tab=stats','/backups/','/calc2/'])
...
def test_all_dropdowns_are_themed_combos(client):
    """قاعدة 10 المقوّاة: ممنوع datalist خام/list= أو select أصلي أبيض — كل قائمة كومبو."""
    for path in ['/recruits/?tab=journal', '/rations/tamween', '/rations/contractor?tab=dist']:
        page = client.get(path).text
        assert '<select' not in page, path              # لا قائمة أصلية على الواجهة
        assert re.search(r'\slist="', page) is None, path  # ممنوع list= الخام
    journal = client.get('/recruits/?tab=journal').text
    assert 'data-combo="rcStList"' in journal            # حالات اليومية كومبو
    copy_page = client.get('/rations/tamween').text
    assert 'data-combo="rtCopyYears"' in copy_page and 'data-combo="rtCopyMonths"' in copy_page
