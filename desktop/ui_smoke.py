...
Promise.all([400,500,600,700].map(weight =>
  ...
  return {
    ...
    fonts: [400,500,600,700].every(weight =>
      document.fonts.check(weight + ' 16px "IBM Plex Sans Arabic"', 'المجندين ٠٫١٢٠')),
    red: getComputedStyle(document.querySelector('.windowbar, .bar')).backgroundColor,
    yellow: button.backgroundColor,
    text: button.color
  };
})
"""
    ...
    def check_theme(then):
        def checked(result):
            if complete.is_set():
                return
            try:
                assert "IBM Plex Sans Arabic" in result["family"] and result["fonts"], result
                assert result["red"] == "rgb(218, 41, 28)", result
                assert result["yellow"] == "rgb(255, 199, 44)", result
                assert result["text"] == "rgb(0, 0, 0)", result
                then()
