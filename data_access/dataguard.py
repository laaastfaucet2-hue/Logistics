...
REASON_LABELS = {
    ...
    "pre-copy": "أمان قبل النسخ بين الشهور",
    "pre-migration": "أمان قبل استيراد النسخة القديمة",
}
...
def data_files():
    ...
    for p in sorted(DATA_DIR.rglob("*")):
        ...
        if "backups" in p.relative_to(DATA_DIR).parts:
            continue
        if p.name.endswith(("-wal", "-shm", ".tmp")) or ".tmp-" in p.name or ".corrupt-" in p.name:
            continue
        out.append(p)
    return out
...
def verify_file(path):
    ...
    try:
        if name.endswith(".db"):
            conn = database.get_conn(path, readonly=True)
...
def create_backup(reason="manual"):
    ...
    name = f"backup_{ts}_{reason}.zip"
    dest = backup_dir() / name
    i = 1
    while dest.exists():
        name = f"backup_{ts}_{i}_{reason}.zip"
        dest = backup_dir() / name
        i += 1
    files = data_files()
    def write_archive(tmp):
        with tempfile.TemporaryDirectory(prefix="logistics-backup-") as scratch:
            with zipfile.ZipFile(tmp, "w", zipfile.ZIP_DEFLATED) as zf:
                for p in files:
                    source = p
                    if p.suffix.lower() == ".db":
                        source = Path(scratch) / "snapshot.db"
                        src, dst = database.get_conn(p, readonly=True), database.get_conn(source)
                        try:
                            src.backup(dst)
                        finally:
                            dst.close()
                            src.close()
                    zf.write(source, p.relative_to(DATA_DIR).as_posix())
    atomic_save(write_archive, dest)
    rotate()
...
def _safe_members(zf):
    ...
    for n in zf.namelist():
        if n.endswith("/") or n.startswith("/") or ".." in n.split("/") or "\\" in n or ":" in n:
            continue
