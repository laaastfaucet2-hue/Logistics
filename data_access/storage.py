...
from core.config import SECTIONS, month_folder, section_folder
...
ARCHIVE_DIR_NAME = "الأرشيف"


def archive_dir():
    """فولدر الأرشيف: database/الأرشيف — خارج التعرف على السنوات (لا يطابق سنة)."""
    return DATA_DIR / ARCHIVE_DIR_NAME


def archive_year(year):
    """ينقل مجلد السنة إلى الأرشيف بنفس البنية. False لو غير موجودة أو مؤرشفة مسبقًا."""
    src = year_path(year)
    if not src.is_dir():
        return False
    dest_root = archive_dir()
...
def letterhead_dir(year, month):
    """مجلد الدباجة والتوقيعات الرسمية: اللوجو + ملف Word الدباجة."""
    path = year_path(year) / month_folder(month) / "الدباجة"
...
def recruits_dir(year, month, category="ملفات"):
    index = next(i for i, section in enumerate(SECTIONS, 1) if section["key"] == "mojandeen")
    path = section_files_path(year, month, index) / category
    path.mkdir(parents=True, exist_ok=True)
    return path
