...
def list_entities(year, month, section=None):
    conn = months.get_db(year, month)
    if section:
        rows = conn.execute(
            "SELECT * FROM entities WHERE section=? ORDER BY serial", (section,))
    else:
        rows = conn.execute("SELECT * FROM entities ORDER BY serial")
    entities = [dict(r) for r in rows.fetchall()]
    for e in entities:
...
def add_entity(year, month, section, name):
    ...
    conn = months.get_db(year, month)
    serial = conn.execute(
        "SELECT COALESCE(MAX(serial),0)+1 s FROM entities WHERE section=?",
        (section,)).fetchone()["s"]
    cur = conn.execute("INSERT INTO entities (name, serial, section) VALUES (?,?,?)",
                       (name, serial, section))
