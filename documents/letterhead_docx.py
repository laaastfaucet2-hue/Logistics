...
يحفظ في: database/<سنة>/<شهر>/الدباجة/الدباجة والتوقيعات.docx
...
def docx_path(year, month):
    return storage.letterhead_dir(year, month) / "الدباجة والتوقيعات.docx"
...
def rebuild(year, month):
    ...
    for i, size in ((1, 16), (2, 14), (3, 13), (4, 13)):
        text = db.get_setting(year, month, f"lh_{i}")
        if text:
    ...
    table.autofit = True
    _sig_cell(table.cell(0, 0), db.get_setting(year, month, "sig_left_rank"),
              db.get_setting(year, month, "sig_left_name"))
    _sig_cell(table.cell(0, 1), db.get_setting(year, month, "sig_right_rank"),
              db.get_setting(year, month, "sig_right_name"))
