...
from core.config import (SECTIONS, RATION_KINDS, RATION_KIND_MAP,
...
def _signatures(ws, row, ncols, year, month):
    ...
    right_rank = db.get_setting(year, month, "sig_right_rank")
    right_name = db.get_setting(year, month, "sig_right_name")
    left_rank = db.get_setting(year, month, "sig_left_rank")
    left_name = db.get_setting(year, month, "sig_left_name")
...
def _kind_sheet(wb, name, year, month, short, section_name, kind_key):
    ...
    next_row = _rows(ws, DATA_ROW, rows) + 2
    _signatures(ws, next_row, len(headers), year, month)
    ws.print_area = f"A1:{__import__('openpyxl').utils.get_column_letter(len(headers))}{next_row + 1}"
...
def _letterhead_revision(year, month):
    return hashlib.sha256(json.dumps(db.get_all(year, month), sort_keys=True).encode()).hexdigest()
...
def ensure(year, month, short):
    ...
    if not stale:
        ...
        try:
            stale = (wb.properties.version != EXPORT_VERSION or
                     wb.properties.keywords != _letterhead_revision(year, month))
        finally:
            wb.close()
    if stale:
...
def rebuild_all():
    ...
    """
    from data_access import months as _months
    count = 0
