...
"""ملفات الوورد الرسمية للمجندين — محلية لحظية في database/<سنة>/<شهر>/08-المجندين/مستندات/:
...
DATE_EXPORT_VERSION = "2.1-day-first-dates"
...
def _header(doc, title, year, month):
    logo = db.get_setting(year, month, "logo_file")
    logo_path = storage.letterhead_dir(year, month) / logo if logo else None
    if logo_path and logo_path.exists():
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.LEFT
        p.add_run().add_picture(str(logo_path), width=Cm(3))
    for i, size in ((1, 15), (2, 13), (3, 12), (4, 12)):
        text = db.get_setting(year, month, f"lh_{i}") or ""
        if text:
            p = doc.add_paragraph()
...
def _signatures(doc, year, month):
    ...
    for idx, side in ((0, "left"), (1, "right")):
        ...
        for key in ("rank", "name"):
            ...
            p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            _run(p, db.get_setting(year, month, f"sig_{side}_{key}") or "........................", 12)
...
def rebuild_month(year, month):
    ...
    from data_access import db_recruits as dr  # تأخر الاستيراد لتفادي الدورات
    ...
    doc = Document()
    doc.core_properties.version = DATE_EXPORT_VERSION
    _landscape(doc)
    _header(doc, title_m, year, month)
    ...
    path1 = docs_dir(year, month) / "كشف-الحالات-{}-{:02d}.docx".format(year, month)
    dataguard.atomic_save(doc.save, path1)

    # ---- كشف إجازات الشهر ----
    doc2 = Document()
    doc2.core_properties.version = DATE_EXPORT_VERSION
    _header(doc2, "كشف الإجازات — شهر {} {}".format(
        MONTH_NAMES[month - 1], arnum.to_arabic_indic(year)), year, month)
    ranges = da.leave_ranges(year, month, matrix, 1, eom)
    by_id = {r["id"]: r for r in recruits}
    t2 = doc2.add_table(rows=1, cols=5)
    t2.style = "Table Grid"
    t2.alignment = WD_TABLE_ALIGNMENT.CENTER
    for j, h in enumerate(("م", "اسم المجند", "الرقم العسكري", "المدة", "من تاريخ — إلى تاريخ")):
    ...
    for rid, rs in ranges.items():
        ...
        for f, t in rs:
            ...
            _fill(cells[4], "{} — {}".format(dates.document_date(date(year, month, f)), dates.document_date(date(year, month, t))), 9)
    if i == 0:
...
def build_leave_permit(recruit, from_day, to_day, year, month):
    ...
    doc = Document()
    doc.core_properties.version = DATE_EXPORT_VERSION
    _header(doc, "تصريح إجازة", year, month)
    lines = [
        ...
        "بإجازة اعتيادية من تاريخ {} وحتى تاريخ {} (مدة {} يومًا).".format(
            dates.document_date(date(year, month, from_day)),
            dates.document_date(date(year, month, to_day)),
            arnum.to_arabic_indic(to_day - from_day + 1)),
        "وعلى المجند المذكور العودة إلى وحدة التعيينات فور انتهاء الإجازة،",
        "وقد حُرِّر له هذا التصريح لتقديمه لمن يهمه الأمر.",
        "حُرِّر في {} — {}.".format(dates.document_date(today), egtime.weekday_ar(today)),
    ]
    for text in lines:
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.RIGHT
...
def ensure_month_sheets(year, month):
    """Regenerate stale monthly reports on explicit download, not while viewing data."""
    pair = month_sheets(year, month)
    if any(path is None or _date_format_is_old(path) for path in pair):
        return rebuild_month(year, month)
    return pair


def _date_format_is_old(path):
    try:
        with zipfile.ZipFile(path) as archive:
            xml = ElementTree.fromstring(archive.read("docProps/core.xml"))
        version = xml.find("{http://schemas.openxmlformats.org/package/2006/metadata/core-properties}version")
        return version is None or version.text != DATE_EXPORT_VERSION
