...
"""
from core import dates, egtime, arabic_numbers as arnum
...
def _time_h(epoch):
    return dates.format_datetime(egtime.from_timestamp(epoch), seconds=True)
...
def create():
    try:
        ...
        return _rb(ok="تم إنشاء نسخة احتياطية بتاريخ {} — {} ملفًا بحجم {}".format(
            _time_h(info["path"].stat().st_mtime), arnum.to_arabic_indic(info["count"]), arnum.to_arabic_indic(_size_h(info["size"]))))
...
def restore(name):
    try:
        ...
        return _rb(ok="تم استرجاع {} ملفًا من النسخة المختارة، مع حفظ نسخة أمان للبيانات السابقة".format(
            arnum.to_arabic_indic(restored)))
...
def delete(name):
    try:
        ...
        return _rb(ok="تم حذف النسخة الاحتياطية المختارة")
