...
from core.config import (SECTIONS, RATION_KINDS, RATION_KIND_MAP, MEALS, DAYS,
                    MONTH_NAMES)
...
def _page_vars(section, kind):
    ...
    return {
        ...
        "meals": MEALS,
        "days": DAYS,
        "units": dr.collect_units(year, month),
...
def page(section):
    ...
    if cfg["has_dist"] and request.args.get("tab") == "dist":
        v["entities"] = de.list_entities(year, month, section)
...
def item_add(section):
    ...
    dr.add_item(year, month, section, kind, name, unit,
...
def custom_save(section, item_id):
    ...
    if not item or item.get("section") != section:
        return _rb(section, kind=kind, err="الصنف غير موجود — افتح التخصيص من صفّه في الجدول")
    entries = []
    for d in range(7):
        for meal, _label in MEALS:
            qty = arnum.parse_float(request.form.get(f"c_{d}_{meal}", ""))
            if qty is not None and qty > 0:          # الفاضي/صفر = لا يُصرف ذلك اليوم
                entries.append((d, meal, qty))
    dr.save_custom(year, month, item_id, entries)
    xlsx_rations.rebuild(year, month, section)
    if entries:
        ok = (f"تخصيص «{item['name']}»: يُصرف في "
              f"{arnum.to_arabic_indic(len(entries))} خانة فقط — "
              "باقي الأيام بلا صرف. آلة 2 مخازن ستلتزم بهذه الأيام.")
    else:
        ok = (f"«{item['name']}» عاد للصرف اليومي "
              "من أرقام الفطار/الغداء/العشاء في كل أيام الأسبوع.")
    return _rb(section, kind=kind, ok=ok)
...
def copy_kind(section):
    ...
    if to_month is None:                                # كومبو الشهر يرسل الاسم العربي — نفكّه هنا
        to_month_raw = (request.form.get("to_month") or "").strip()
        if to_month_raw in MONTH_NAMES:
            to_month = MONTH_NAMES.index(to_month_raw) + 1
...
# ======================================================================
# الجهات (تاب توزيع المقررات — التموين والمتعهد، كل قسم بجهاته)
# ======================================================================
...
def eitem_add(section, entity_id):
    ...
    unit = (request.form.get("unit") or "").strip()
    de.add_entity_item(year, month, entity_id, name, unit,
                       arnum.parse_float(request.form.get("breakfast")),
                       arnum.parse_float(request.form.get("lunch")),
                       arnum.parse_float(request.form.get("dinner")))
    dr.remember_unit(unit)
...
def eitem_edit(section, item_id):
    ...
    if not name:
        return _rb(section, tab="dist", err="اكتب اسم الصنف أولًا")
    unit = (request.form.get("unit") or "").strip()
    de.update_entity_item(year, month, item_id, name, unit,
                          arnum.parse_float(request.form.get("breakfast")),
                          arnum.parse_float(request.form.get("lunch")),
                          arnum.parse_float(request.form.get("dinner")))
    dr.remember_unit(unit)
